...
public class GradientDrawer : PropertyDrawer
{
    ...
    {
        Event guiEvenet = Event.current;

        CustomGradient gradient =
            (CustomGradient)fieldInfo.GetValue(property.serializedObject.targetObject);

        float labelWidth = GUI.skin.label.CalcSize(label).x + 5.0f;
        Rect textureRect = new Rect(position.x + labelWidth, position.y, position.width - labelWidth, position.height);

        if (guiEvenet.type == EventType.Repaint)
        {
        ...
        {
            if (guiEvenet.type == EventType.MouseDown && guiEvenet.button == 0)
            {
                if (textureRect.Contains(guiEvenet.mousePosition))
                {
